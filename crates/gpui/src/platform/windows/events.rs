...
pub(crate) fn handle_msg(
    ...
    let handled = match msg {
        ...
        WM_MOVE => (!state_ptr.ignore_windows_events).then(|| handle_move_msg(handle, lparam, state_ptr)).flatten(),
        WM_SIZE => (!state_ptr.ignore_windows_events).then(|| handle_size_msg(wparam, lparam, state_ptr)).flatten(),
        WM_GETMINMAXINFO => handle_get_min_max_info_msg(lparam, state_ptr),
        WM_ENTERSIZEMOVE | WM_ENTERMENULOOP => (!state_ptr.ignore_windows_events).then(|| handle_size_move_loop(handle)).flatten(),
        WM_EXITSIZEMOVE | WM_EXITMENULOOP => (!state_ptr.ignore_windows_events).then(|| handle_size_move_loop_exit(handle)).flatten(),
        ...
        WM_PAINT => (!state_ptr.ignore_windows_events).then(|| handle_paint_msg(handle, state_ptr)).flatten(),
